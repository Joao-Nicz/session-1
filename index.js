...
const renderBookFigure = (bookParam) => `
    <figure class="fig-book">
        <div class="fig-img">
            <img src="./img/books/${bookParam.id +'.png'?? 'missing.png'}" alt="Book Image"/>
        </div>
        <figcaption>
...
const onBookClick = (bookId, bookType) => {
  bookFocus(bookId, bookType);
};

const bookFocus = (bookId, bookType) => {
  const book = document.querySelector(`*[data-id="${bookId}"] .fig-book`);
  book.classList.toggle('active');

  // Some active state DONE
  // - Book to right DONE
  // - Text to left DONE 
  // Description show everything DONE
  // Description scroll
